...
# ============================================
# Iara Code Reviewer - Composite Action Script
# ============================================
# This script runs Iara directly on the GitHub Actions host runner,
# replacing the Docker-based entrypoint for better performance and
# compatibility with pip caching.

# --- Input Variables (from action.yml inputs, mapped via env) ---
...
if [ "$INDEX_CODEBASE" = "true" ]; then
  ...
  cd "$WORKSPACE"
  python3 -m iara memory index || echo "::warning::Failed to index codebase via RAG."
fi

# --- Run Iara ---
cd "$WORKSPACE"
REVIEW=$(python3 -m iara 2>/tmp/iara_stderr.txt || true)
